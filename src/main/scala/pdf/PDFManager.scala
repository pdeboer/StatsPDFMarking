	...
	def highlightFile(pdfFile: File): List[Option[List[Permutation]]] = {
		...
		terms.methods.par.map(method => {
			...
			try {
				...
				//TODO createstatmethodlist takes many parameters. It would be cleaner to have a new class "PaperHighlightManager" where permuter, delta, method list etc are member variables and you could call everything directly
